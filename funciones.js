...
/**
 * Sistema de Descarga de Postal 3D (Frente y Dorso) - Versión Segura
 */
// Encapsulamos todo dentro de DOMContentLoaded para asegurar que el botón ya exista al ejecutar el script
document.addEventListener("DOMContentLoaded", function () {
    
    const botonDescargar = document.getElementById('btn-descargar');
    
    if (!botonDescargar) {
        console.error("Error: No se encontró ningún botón con el id 'btn-descargar'.");
        return;
    }

    botonDescargar.addEventListener('click', async function(e) {
        e.preventDefault(); // Detiene cualquier comportamiento por defecto del enlace

        // 1. Identificamos las caras en el HTML
        const caraFrente = document.querySelector('.cara-frente');
        const caraAtras = document.querySelector('.cara-atras');

        if (!caraFrente || !caraAtras) {
            alert("Error visual: No se encontraron las clases de las caras (.cara-frente o .cara-atras)");
            return;
        }

        // 2. CORRECCIÓN CLAVE: Forma universal de llamar a jsPDF según la versión cargada (UMD/Cdnjs)
        let ConstructorPDF;
        if (window.jspdf && window.jspdf.jsPDF) {
            ConstructorPDF = window.jspdf.jsPDF;
        } else if (window.jsPDF) {
            ConstructorPDF = window.jsPDF;
        } else {
            alert("La librería jsPDF no ha cargado correctamente en la página.");
            return;
        }
        ...
        try {
            // Indicador opcional de carga en consola
            console.log("Procesando descarga de postal...");

            // --- PASO 1: Capturar la Cara Frontal ---
            const canvasFrente = await html2canvas(caraFrente, opcionesCaptura);
            const imgFrente = canvasFrente.toDataURL('image/png');

            // --- PASO 2: Capturar la Cara Trasera (Manejando el 3D) ---
            const estiloOriginalTransform = caraAtras.style.transform;
            const estiloOriginalVisibility = caraAtras.style.backfaceVisibility;

            caraAtras.style.transform = 'none';
            caraAtras.style.backfaceVisibility = 'visible';

            const canvasAtras = await html2canvas(caraAtras, opcionesCaptura);
            const imgAtras = canvasAtras.toDataURL('image/png');

            // Restauramos el CSS 3D al instante
            caraAtras.style.transform = estiloOriginalTransform;
            caraAtras.style.backfaceVisibility = estiloOriginalVisibility;

            // --- PASO 3: Construcción del PDF ---
            const anchoPdf = canvasFrente.width;
            const altoPdf = canvasFrente.height;

            const pdf = new ConstructorPDF({
                orientation: anchoPdf > altoPdf ? 'l' : 'p', 
                unit: 'px',
                format: [anchoPdf, altoPdf]
            });

            // Página 1: Frente
            pdf.addImage(imgFrente, 'PNG', 0, 0, anchoPdf, altoPdf);

            // Página 2: Dorso
            pdf.addPage([anchoPdf, altoPdf]);
            pdf.addImage(imgAtras, 'PNG', 0, 0, anchoPdf, altoPdf);

            // Forzar descarga del archivo
            pdf.save('Mi_Postal_Personalizada.pdf');
            console.log("¡Descarga completada!");

        } catch (error) {
            console.error("Error crítico durante la generación del PDF:", error);
            alert("Hubo un error al procesar el render de las imágenes.");
        }
    });
});
